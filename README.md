...
- 回顾：连续记录、活跃日历、全年热力图、分类占比、情绪趋势、周月总结与带来源问答。
- 日历：月历上点某天看当天安排；事件支持重复、每事件自选提醒铃声，也可以用一句话安排（「这周五八点记得去上实验课」）。
- 数据：单条导出 Markdown、按筛选结果批量导出成一个带目录的文件、每天自动备份到本机（保留最近 3 份并可一键恢复）。
